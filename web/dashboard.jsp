...
<head>
    ...
    <style>
        * { margin: 0;
padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
...
gap: 15px;
        ...
        .logo { font-size: 24px; font-weight: 800; color: #6366f1; display: flex; align-items: center;
...
border-radius: 10px; color: #64748b; transition: 0.3s; font-weight: 500; text-decoration: none; white-space: nowrap;
}
...
grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 25px; }
        .m-card { background: #ffffff; border-radius: 20px;
padding: 25px; text-align: center; border: 1px solid rgba(0,0,0,0.02); transition: 0.3s; cursor: pointer;
}
        ...
        .m-avatar { width: 80px;
height: 80px; border-radius: 50%; margin: 0 auto 15px; background: #6366f1; display: flex; align-items: center; justify-content: center; font-size: 28px; color: white;
overflow: hidden; border: 3px solid white; box-shadow: 0 4px 10px rgba(0,0,0,0.1); position: relative;
}
        .m-avatar img { width: 100%; height: 100%; object-fit: cover;
}
        
        .btn { padding: 10px 20px;
border-radius: 10px; font-weight: 600; cursor: pointer; border: none; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; font-size: 14px; transition: 0.2s;
}
        ...
        .detail-box { background: white;
padding: 40px; border-radius: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.04); max-width: 900px; margin: 0 auto;
}
        .request-table { width: 100%; border-collapse: separate; border-spacing: 0 10px; margin-top: 20px;
}
        .request-table th { padding: 12px 20px; text-align: left; color: #94a3b8; font-size: 12px;
text-transform: uppercase; letter-spacing: 1px; }
        .request-table tr td { background: white;
padding: 15px 20px; transition: 0.2s; }
        .request-table tr td:first-child { border-radius: 12px 0 0 12px;
}
        .request-table tr td:last-child { border-radius: 0 12px 12px 0;
}
        
        .small-avatar { width: 45px;
height: 45px; border-radius: 50%; overflow: hidden; background: #6366f1; color: white; display: flex; align-items: center; justify-content: center; font-size: 18px;
border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        .small-avatar img { width: 100%;
height: 100%; object-fit: cover; }
        
        .info-pill { padding: 4px 12px;
background: #eef2ff; color: #6366f1; border-radius: 20px; font-size: 12px; font-weight: 600;
}
        input, textarea, select { width: 100%; padding: 12px; border: 1px solid #e2e8f0;
border-radius: 10px; outline-color: #6366f1; }
...
<div class="sidebar" id="sidebar">
   ...
   <li>
        ...
        <li>
          ...
          <a href="dashboard.jsp?view=profile" class="<%= view.equals("profile") ?
"active" : "" %>">
...
margin-top: auto;">
        <div style="padding: 12px 15px; color: #f87171; font-weight: 600; display: flex;
...
font-weight: 700;">Welcome back, <%= me.getStudentName() %></h2>
    </div>
    <div style="display: flex; align-items: center;
gap: 15px;">
        <div style="text-align: right;">
            <div style="font-weight: 600;
font-size: 14px;"><%= me.getStudentName() %></div>
            <div style="font-size: 11px; color: #6366f1;
font-weight: 700;"><%= currentRole.toUpperCase() %></div>
        </div>
        <div class="small-avatar" style="width: 40px;
height: 40px;">
...
s.getCourseCode().toLowerCase() %>"
                    ...
                    <h3 class="mentor-name" style="font-size: 18px;
margin-bottom: 5px;"><%= s.getStudentName() %></h3>
...
text-align: center; padding: 50px; color: #94a3b8;">
            <i class="fa-solid fa-user-slash" style="font-size: 48px;
margin-bottom: 10px;"></i>
            ...
            <h2 style="margin-bottom: 25px;">My Connections</h2>
            ...
            <div class="grid">
                ...
                <div class="m-card" onclick="location.href='dashboard.jsp?view=connection_detail&mid=<%= person.getStudentID()%>'">
                    ...
                    <div class="m-avatar">
                        ...
                        <% if (person.getProfilePic() != null) {%><img src="<%= person.getProfilePic()%>"><% } else {%><%= person.getStudentName().substring(0, 1)%><% }%>

                    </div>

                    <h3><%= person.getStudentName()%></h3>

                    <p style="color: #64748b; font-size: 14px; margin: 5px 0;"><%= person.getCourseCode()%></p>

                    <span class="info-pill" style="background: #ecfdf5; color: #10b981;">
                ...
                <div style="grid-column: 1/-1; text-align: center; padding: 60px; color: #94a3b8;">
                    ...
                    <i class="fa-solid fa-user-group" style="font-size: 48px; margin-bottom: 15px; opacity: 0.3;"></i>
            ...
            <a href="dashboard.jsp?view=connections" class="btn btn-outline" style="margin-bottom: 20px;"><i class="fa-solid fa-arrow-left"></i> Back to Connections</a>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">

                <div class="detail-box" style="margin: 0; max-width: none;">

                    <div style="display: flex; gap: 30px; align-items: flex-start; margin-bottom: 30px;">

                        <div class="m-avatar" style="width: 120px; height: 120px; font-size: 48px; margin:0;">
                        ...
                        <div style="flex-grow: 1;">
                            ...
                            <h1 style="font-size: 28px;"><%= person.getStudentName()%></h1>

                            <span class="info-pill" style="background: #ecfdf5; color: #10b981; margin: 8px 0; display: inline-block;">Approved Connection</span>

                            <p style="color: #64748b;"><i class="fa-solid fa-envelope"></i> <%= person.getStudentEmail()%></p>
                            <p style="color: #64748b;"><i class="fa-solid fa-phone"></i> <%= person.getStudentPhone()%></p>
                    ...
                    <div style="border-top: 1px solid #f1f5f9; padding-top: 20px;">
                        ...
                        <h4 style="margin-bottom: 10px; font-size: 12px; text-transform: uppercase; color: #94a3b8;">About</h4>

                        <p><%= (person.getStudentBio() != null && !person.getStudentBio().isEmpty()) ? person.getStudentBio() : "Professional profile details for " + person.getStudentName() + "."%></p>
 ...
 <div style="display: flex; gap: 40px; align-items: flex-start;">
                <div class="m-avatar" style="width: 150px;
height: 150px; margin: 0; font-size: 60px;">
                ...
                <div style="flex-grow: 1;">
                    <h1 style="font-size: 
32px; margin-bottom: 5px;"><%= mentor.getStudentName() %></h1>
                    <div style="display: flex;
gap: 10px; margin-bottom: 20px;">
                        <span class="info-pill"><%= mentor.getCourseCode() %></span>
                        <span class="info-pill" style="background: #eef2ff;
color: #6366f1;">VERIFIED MENTOR</span>
                    </div>
                    
                    <div style="display: grid;
grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 25px;">
                        <div style="padding: 15px;
background: #f8fafc; border-radius: 12px;">
                            <label>Email Address</label>
                            <div style="font-size: 14px;
font-weight: 600;"><%= mentor.getStudentEmail() %></div>
                        </div>
                         <div style="padding: 15px;
background: #f8fafc; border-radius: 12px;">
                            <label>Phone Number</label>
                            <div style="font-size: 14px;
font-weight: 600;"><%= mentor.getStudentPhone() %></div>
                        </div>
                    </div>
                  ...
                  <p style="line-height: 1.6;"><%= (mentor.getStudentAchievements()!=null && !mentor.getStudentAchievements().isEmpty())?mentor.getStudentAchievements():"No achievements listed." %></p>
                    ...
                    <div style="display: flex;
gap: 12px; margin-top: 30px; border-top: 1px solid #f1f5f9; padding-top: 25px;">
    ...
    <% } else if (view.equals("requests")) { %>
        ...
        <div class="detail-box" style="max-width: 1000px;
padding: 20px;">
...
padding: 50px; color: #94a3b8;">No pending requests.</td></tr>
                    <% } else { for (Map<String, Object> req : requests) { %>
                    <tr>
                        <td style="display: flex;
align-items: center; gap: 15px; cursor: pointer;" onclick="location.href='dashboard.jsp?view=mentee_detail&mid=<%= req.get("menteeId") %>'">
...
color: #1e293b;"><%= req.get("menteeName") %></div>
                                <div style="font-size: 11px;
color: #6366f1;">Click to view profile</div>
                            </div>
                        </td>
...
height: 150px; margin: 0; font-size: 60px;">
                ...
                <div style="flex-grow: 1;">
                    <h1 style="font-size: 
32px; margin-bottom: 5px;"><%= mentee.getStudentName() %></h1>
                    <div style="display: flex;
gap: 10px; margin-bottom: 20px;">
                        <span class="info-pill"><%= mentee.getCourseCode() %></span>
                        <span class="info-pill" style="background: #f0fdf4;
color: #16a34a;">MENTEE APPLICANT</span>
                    </div>
...
font-weight: 800; color: #10b981;"><%= mentee.getStudentCGPA() %></div>
                        </div>
                        <div style="padding: 15px;
background: #f8fafc; border-radius: 12px;">
                            <label>Email Address</label>
                            <div style="font-size: 14px;
font-weight: 600;"><%= mentee.getStudentEmail() %></div>
                        </div>
                    </div>
    ...
    <% } else if (view.equals("profile")) { %>
        ...
        <div class="detail-box">
          ...
          <form action="ProfileServlet" method="POST" enctype="multipart/form-data">
                ...
                <div style="display: flex;
align-items: flex-start; gap: 40px; margin-bottom: 30px;">
                    <div style="text-align: center;">
                        <div class="m-avatar" style="width: 130px;
height: 130px; font-size: 45px; margin-bottom: 15px;">
                            <% if (me.getProfilePic() != null && !me.getProfilePic().isEmpty()) { %><img src="<%= me.getProfilePic() %>"><% } else { %><%= me.getStudentName().substring(0,1) %><% } %>
                        </div>
...
<label class="btn btn-outline" style="font-size: 12px; cursor: pointer; text-transform: none; letter-spacing: 0;">
                    ...
                    <div style="flex-grow: 1;">
                        <div style="display: grid;
grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
...
for(Map<String, String> c : courses) { %>
                                         <option value="<%= c.get("code") %>" <%= c.get("code").equals(me.getCourseCode()) ?
"selected" : "" %>>
...
padding: 20px; border: 2px solid #fee2e2; border-radius: 12px; background: #fff;">
    <h3 style="color: #991b1b; font-size: 18px;
margin-bottom: 8px;">Delete Account</h3>
...
margin-bottom: 15px;">
    ...
    <form action="ProfileServlet" method="POST" onsubmit="return confirm('WARNING: This will drop ALL your data from the database. Are you sure?');">
       ...
       <button type="submit" style="background: #dc2626; color: white; border: none; padding: 12px 24px; border-radius: 8px;
